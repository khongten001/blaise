...
* **Testing:** 4275+ tests and growing (Test-Driven Development from day one). The test suite itself compiles under Blaise, and runs in CI on both Linux and FreeBSD.
...
* **Standard library:** A growing opt-in stdlib — generics collections, JSON (DOM + parser + writer), SHA-1 and Base64, RFC 4122 UUIDs, TCP sockets, WebSockets, a minimal HTTP/1.1 server, and a `blaise.testing` unit-test framework.
...
once (`cd vendor/qbe && make`) and replace step 2 with an emit-assemble-link
sequence:

[source,shell]
----
# 2a. Emit whole-program QBE IR and assemble it to an object.
$RELEASE \
  --source compiler/src/main/pascal/Blaise.pas \
  --unit-path compiler/src/main/pascal \
  --unit-path runtime/src/main/pascal \
  --unit-path stdlib/src/main/pascal \
  --backend qbe --emit-ir > /tmp/blaise.ssa
vendor/qbe/qbe -o /tmp/blaise.s /tmp/blaise.ssa
gcc -c -o /tmp/blaise.o /tmp/blaise.s

# 2b. Build the RTL objects from source and link.  The whole-program IR INLINES
#     the RTL units it uses, so --exclude-defined-by drops the RTL objects whose
#     symbols /tmp/blaise.o already defines — otherwise they double-define at the
#     link.  (This mirrors what the compiler's own driver and scripts/fixpoint.sh
#     do; do NOT link against blaise_rtl.a directly — that archive is built for
#     the fixpoint scripts and the runtime test module, and its members redefine
#     the same inlined RTL symbols, so a whole-program QBE link against it fails
#     with multiple-definition errors.)
RTL=$(scripts/build-rtl-objects.sh "$RELEASE" /tmp/rtlobj \
        --exclude-defined-by /tmp/blaise.o)
gcc -o compiler/target/blaise /tmp/blaise.o $RTL -lm -lpthread
----
...
Standalone Blaise projects that show the language, the async/fibre runtime, and
the standard library used in real applications:

* https://github.com/graemeg/blaise-microservice-demo[blaise-microservice-demo] —
  a port of the Java Virtual Threads "API aggregator" demo. An HTTP server whose
  `/aggregate` endpoint fans out three concurrent downstream calls that complete
  in ~2 s wall-clock (not 6 s) via the fibre `TTaskGroup` nursery and the M:N
  scheduler, plus a mini-Gatling load generator that ramps to 6000 concurrent
  virtual users on a handful of OS threads — no framework, no GC. Includes a
  side-by-side Blaise-vs-Java comparison.

* https://github.com/graemeg/luhmann[luhmann] — a
  https://en.wikipedia.org/wiki/Zettelkasten[Zettelkasten] note server. Turns a
  directory of AsciiDoc notes into a linked web of pages served over HTTP, with
  an interactive note graph and a JSON API, rendering the whole collection
  concurrently at start-up.
