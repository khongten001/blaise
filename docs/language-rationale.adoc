...
* Iterating over static arrays (`for X in Arr do`) — deferred; requires
  a built-in range-iteration path separate from GetEnumerator.
* Iterating over sets (`for X in [a, b, c] do`) — deferred.
* Byte-wise iteration over strings (`for B in S do`) — deferred; Blaise has
  no `Char` type; byte iteration would yield `Byte` values consistent with
  `S[N]` semantics, but the implementation is not yet present.
